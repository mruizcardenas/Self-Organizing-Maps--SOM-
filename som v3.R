  ...
  if(type=="dist"){
    ...
    for(i in 1:m){
      if(sum((cmin-finalnet$coord[i,])^2)<=1){
        ...
        if(sum((cmin-finalnet$coord[i,])^2)<0.01){
          finalnet$coord[i,]<-cmin
        }else{
          finalnet$coord[i,]<-finalnet$coord[i,]+modfactor*(cmin-finalnet$coord[i,])
        }
        #plot(finalnet$coord[,1],finalnet$coord[,2],main=i)      
      }
    }
    return(finalnet)
    ...
    cmin<-finalnet$coord[ele_mod[1],]
    for(i in ele_mod){
      finalnet$nodo[i,]<-finalnet$nodo[i,]+modfactor*(ele-finalnet$nodo[i,])
      if(sum((cmin-finalnet$coord[i,])^2)<0.01){
        finalnet$coord[i,]<-cmin
      }else{
        finalnet$coord[i,]<-finalnet$coord[i,]+modfactor*0.01*(cmin-finalnet$coord[i,])
      }
    }
...
m<-4
SOM<-som_train(datosprueba,p=9,a=3,epochs = 10,meshtype = "grid",modtype = "dist")
S_0<-SOM$ini$coord
print(S_0)
S_1<-SOM$final$coord
print(S_1)
...
datosint<-read.csv("caso1_1.csv")
SOM<-som_train(datosint,p=50,a=50,epochs =1000 , meshtype = "grid",modtype = "neighbour")
som_clustering(datosprueba,SOM$final$nodo,2)

plot(SOM$ini$coord[,1],SOM$ini$coord[,2])
plot(SOM$final$coord[,1],SOM$final$coord[,2])
